...
CACHE_ENABLED = getattr(configs, "FIREBASE_AUTH_CACHE_ENABLED", True)
CACHE_TTL_DEFAULT = getattr(configs, "FIREBASE_AUTH_CACHE_TTL_SECONDS", 300)
CACHE_PREFIX = getattr(configs, "FIREBASE_AUTH_CACHE_PREFIX", "firebase:id_token")
REDIS_CACHE_DB = getattr(configs, "REDIS_CACHE_DB", None)
...
def verify_id_token_with_cache(token: str, app_instance, cache_prefix: str = CACHE_PREFIX) -> Dict[str, Any]:
    ...
    if CACHE_ENABLED:
        redis_instance = RedisJSONWrapper(database=REDIS_CACHE_DB)
