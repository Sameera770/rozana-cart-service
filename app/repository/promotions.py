...
class PromotionsRepository:
    ...
    async def get_rto_count_for_user(self, user_id: str) -> int:
        try:
            query = "SELECT COUNT(*) as count FROM returns WHERE customer_id = :user_id AND return_type = 'RTO'"
            rows = execute_raw_sql_readonly(query, {"user_id": user_id})
            logger.info(f"get_rto_count_for_user_result | user_id={user_id} count={rows}")
            return rows[0].get("count", 0) if rows else 0
        except Exception as e:
